  ...
  dim_sty.name = strpool_inject( &name_pool, "STANDARD", strlen("STANDARD") );
	...
	while (current){
		if (current->type == DXF_ATTR){ /* DXF attibute */
          ...
          { const char * str = strpool_cstr( &value_pool, current->value.str);
	...
	if (dim_sty.tstyle >= 0){
      ...
      (void *) strpool_cstr( &name_pool, drawing->text_styles[dim_sty.tstyle].name));
	}
...
int dxf_find_last_dim (dxf_drawing *drawing){
	...
	while (current){
      ...
      if (dxf_ident_ent_type(current) == DXF_BLK){
            ...
            strpool_cstr( &name_pool, descr_attr->value.str), DXF_MAX_CHARS);
...
int dxf_dim_get_sty(dxf_drawing *drawing, dxf_dimsty *dim_sty){
    ...
    strpool_inject( &name_pool, "STANDARD", strlen("STANDARD") ));
	dim_sty->obj = NULL;
	
	if (!(dsty_obj = dxf_find_obj_descr2(drawing->t_dimst, "DIMSTYLE", //dim_sty->name)))
    (char*) strpool_cstr( &name_pool, dim_sty->name))))
      return -1; /* not found */
...
int dxf_dim_update_sty(dxf_drawing *drawing, dxf_dimsty *dim_sty){
	...
	if (dim_sty->tstyle >=0) {
        ...
        (char*) strpool_cstr( &value_pool, tsty_obj->value.str));
		}
	}
  ...
  STRPOOL_U64 std = strpool_inject( &name_pool, "STANDARD", strlen("STANDARD"));
	...
	for (i = 0; i< 2; i++){ /* look in BLOCKS and ENTITIES sections */
        ...
        if (dxf_ident_ent_type(current) == DXF_DIMENSION){
              ...
              (char*) strpool_cstr( &name_pool, dsty_name->value.str));
						if(dsty) {
							/* uses DIMSTYLE's layer index to count */
							dsty->obj.layer++;
						}
